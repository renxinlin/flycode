...
{
  int32_t  ADPress; // 表示压力传感器的模拟数字转换值，单位为Pa
  int32_t  ADTemp;  // 表示压力传感器的模拟数字转换值，单位为Pa
  float  RP; // 表示当前气压值
  float  RT; // 表示当前温度值，单位为摄氏度。
  float  RPFilter; // 表示经过滤波器处理后的气压值，单位为Pa。
  float  InitPress; // 表示初始气压值，单位为Pa
  float  Altitude; // ：表示当前海拔高度，单位为米。
  float  AltitudeFilter; // 表示经过滤波器处理后的海拔高度，单位为米。
  uint16_t C0, C1, C2, C3, C6, C8, C9, C10, C11, C12; // 表示校准系数，用于计算气压值和海拔高度。
  uint32_t C4, C5, C7; // 表示校准系数，用于计算气压值和海拔高度。
